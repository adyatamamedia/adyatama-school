    ...
    <div class="col-md-9 text-end">
        <div class="d-inline-flex align-items-center gap-2 justify-content-end">
            ...
            <div class="btn-group btn-group-sm" id="bulkActionsGroup" style="display:none;">
                <button type="button" class="btn btn-success" onclick="confirmBulkAction('restore', 'Restore galeri terpilih?')">
                    <i class="fas fa-trash-restore"></i> Restore (<span class="bulk-count">0</span>)
                </button>
                <button type="button" class="btn btn-danger" onclick="confirmBulkAction('force-delete', 'Hapus PERMANEN galeri terpilih?')">
                    <i class="fas fa-ban"></i> Delete Forever (<span class="bulk-count">0</span>)
                </button>
            </div>

            <!-- Back Button -->
            <a href="<?= base_url('dashboard/galleries') ?>" class="btn btn-secondary btn-sm">
                <i class="fas fa-arrow-left"></i> Kembali ke Galleries
            </a>
        </div>
    </div>
</div>
...
<div class="card shadow-sm mb-4 border-danger border-start border-3">
    <div class="card-header bg-light text-danger">
        <i class="fas fa-trash me-2"></i> <strong>Trash (Deleted Galleries)</strong>
    </div>
    <div class="card-body">
                    ...
                    <?php if (!empty($galleries)) : ?>
                        <?php foreach ($galleries as $gallery) : ?>
                            <tr>
                                ...
                                <td>
                                    <div class="fw-bold text-decoration-line-through text-muted"><?= esc($gallery->title) ?></div>
                                    <small class="text-muted">/<?= esc($gallery->slug) ?></small>
                                </td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <?php if($gallery->author_photo): ?>
                                            <img src="<?= base_url($gallery->author_photo) ?>" alt="<?= esc($gallery->author_name) ?>" class="rounded-circle me-2" style="width: 32px; height: 32px; object-fit: cover;">
                                        <?php endif; ?>
                                        <span><?= esc($gallery->author_name) ?></span>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <small class="text-danger"><?= date('d M Y H:i', strtotime($gallery->deleted_at)) ?></small>
                                </td>
                                <td class="text-center">
                                    <button type="button" class="btn btn-sm btn-outline-success" title="Restore" onclick="showRestoreModal('<?= base_url('dashboard/galleries/restore/' . $gallery->id) ?>', '<?= esc($gallery->title, 'js') ?>')">
...
<div class="row">
    <div class="col-12">
        ...
        <div class="text-center text-muted small mb-3">
            Menampilkan <?= count($galleries) ?> dari <?= $pager->getTotal() ?> galeri di trash
        </div>
    </div>
</div>
<?php endif; ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Apakah Anda yakin ingin mengembalikan galeri <strong id="restoreItemTitle"></strong> dari trash?
            </div>
...
function submitBulkAction() {
    ...
    form.method = 'POST';
    form.action = '<?= base_url('dashboard/galleries/bulk-') ?>' + currentBulkAction; 
