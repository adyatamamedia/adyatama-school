    ...
    <div class="col-md-9 text-end">
        <div class="d-inline-flex align-items-center gap-2 justify-content-end">
            ...
            <div class="btn-group btn-group-sm" id="bulkActionsGroup" style="display:none;">
                <button type="button" class="btn btn-success" onclick="confirmBulkAction('restore', 'Restore pendaftaran terpilih?')">
                    <i class="fas fa-trash-restore"></i> Restore (<span class="bulk-count">0</span>)
                </button>
                <button type="button" class="btn btn-danger" onclick="confirmBulkAction('force-delete', 'Hapus PERMANEN pendaftaran terpilih?')">
                    <i class="fas fa-ban"></i> Delete Forever (<span class="bulk-count">0</span>)
                </button>
            </div>

            <!-- Back Button -->
            <a href="<?= base_url('dashboard/pendaftaran') ?>" class="btn btn-secondary btn-sm">
...
<div class="card shadow-sm mb-4 border-danger border-start border-3">
    <div class="card-header bg-light text-danger">
        <i class="fas fa-trash me-2"></i> <strong>Trash (Deleted Pendaftaran)</strong>
    </div>
    <div class="card-body">
                ...
                <tbody>
                    <?php if (!empty($applications)) : ?>
                        <?php foreach ($applications as $application) : ?>
                            <tr>
                                <td>
                                    <input type="checkbox" class="form-check-input bulk-checkbox" value="<?= $application['id'] ?>" onchange="toggleBulkSelection(this)">
                                </td>
                                <td>
                                    <div class="fw-bold text-decoration-line-through text-muted"><?= esc($application['nama_lengkap']) ?></div>
                                    <?php if($application['nisn']): ?>
                                        <small class="text-muted">NISN: <?= esc($application['nisn']) ?></small>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <span class="badge text-decoration-line-through bg-secondary">
                                        <?= ucfirst(esc($application['jenis_kelamin'])) ?>
                                    </span>
                                </td>
                                <td>
                                    <span class="badge text-decoration-line-through bg-secondary">
                                        <?= ucfirst(esc($application['status'] ?? 'pending')) ?>
                                    </span>
                                </td>
                                <td>
                                    <small class="text-danger"><?= date('d M Y H:i', strtotime($application['deleted_at'])) ?></small>
                                </td>
                                <td class="text-center">
                                    <button type="button" class="btn btn-sm btn-outline-success" title="Restore" onclick="showRestoreModal('<?= base_url('dashboard/pendaftaran/restore/' . $application['id']) ?>', '<?= esc($application['nama_lengkap'], 'js') ?>')">
...
<div class="row">
    <div class="col-12">
        ...
        <div class="text-center text-muted small mb-3">
            Menampilkan <?= count($applications) ?> dari <?= $pager->getTotal() ?> pendaftaran di trash
        </div>
    </div>
</div>
<?php endif; ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Apakah Anda yakin ingin mengembalikan pendaftaran <strong id="restoreItemTitle"></strong> dari trash?
            </div>
...
function submitBulkAction() {
    ...
    form.method = 'POST';
    form.action = '<?= base_url('dashboard/pendaftaran/bulk-') ?>' + currentBulkAction; 
