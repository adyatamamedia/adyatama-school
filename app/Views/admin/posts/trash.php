...
<div class="row mb-3 align-items-center">
    ...
    <div class="col-md-9 text-end">
        <div class="d-inline-flex align-items-center gap-2 justify-content-end">
            ...
            <!-- Back Button -->
            <a href="<?= base_url('dashboard/posts') ?>" class="btn btn-secondary btn-sm">
                <i class="fas fa-arrow-left"></i> Kembali ke Posts
            </a>
        </div>
    </div>
</div>
...
<div class="card shadow-sm mb-4 border-danger border-start border-3">
    <div class="card-header bg-light text-danger">
        <i class="fas fa-trash me-2"></i> <strong>Trash (Deleted Posts)</strong>
    </div>
    <div class="card-body">
                    ...
                    <?php if (!empty($posts)) : ?>
                        <?php foreach ($posts as $post) : ?>
                            <tr>
                                ...
                                <td>
                                    <div class="fw-bold text-decoration-line-through text-muted"><?= esc($post->title) ?></div>
                                ...
                                <td>
                                    <small class="text-danger"><?= date('d M Y H:i', strtotime($post->deleted_at)) ?></small>
                                </td>
                                <td class="text-center">
                                    <button type="button" class="btn btn-sm btn-outline-success" title="Restore" onclick="showRestoreModal('<?= base_url('dashboard/posts/restore/' . $post->id) ?>', '<?= esc($post->title, 'js') ?>')">
...
<div class="row">
    <div class="col-12">
        ...
        <div class="text-center text-muted small mb-3">
            Menampilkan <?= count($posts) ?> dari <?= $pager->getTotal() ?> post di trash
        </div>
    </div>
</div>
<?php endif; ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Apakah Anda yakin ingin mengembalikan post <strong id="restoreItemTitle"></strong> dari trash?
            </div>
...
function submitBulkAction() {
    ...
    form.method = 'POST';
    form.action = '<?= base_url('dashboard/posts/bulk-') ?>' + currentBulkAction; 
